...
For complex widgets that need framework adapters (React, Svelte, Vue) or custom logic, use [Chirp islands](/chirp/docs/guides/islands/). Islands provide a `data-island` mount contract with lifecycle events (`chirp:island:mount`, `chirp:island:unmount`) and optional dynamic adapter loading via `data-island-src`. Islands are client-owned surfaces; the server does not swap HTML inside them.
