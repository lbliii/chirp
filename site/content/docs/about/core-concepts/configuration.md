...
| `mcp_connect_default` | `str | None` | `None` | Optional public Streamable HTTP connect default when `initialize` omits `protocolVersion` or requests an unknown legacy version (e.g. `"2025-06-18"`); `None` keeps Chirp's internal newest default. Known legacy revisions such as `2025-11-25` are echoed back. |
