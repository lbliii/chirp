...
| `skill_contract` | ERROR / WARNING | Mounted `chirp.skill` tools must be signable and correctly scoped: missing signing key or incomplete freeze-time manifest → `ERROR`; unknown required scope (when a scope registry exists) and scoped tools without `AuthMiddleware` → env-aware (`ERROR` production / `WARNING` staging / silent development). Registered by `use_skill` via `register_contract_check`. |
