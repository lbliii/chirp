...
| `csp_nonce` | ERROR | When an app ships a nonce-based / inline-forbidding CSP (`CSPNonceMiddleware`, `csp_nonce_enabled=True`, or a static `script-src` without `'unsafe-inline'`), framework inline `<script>` tags must be nonceable. Enabling `alpine=True` under such a policy is an ERROR: the Alpine `safeData` bootstrap is injected at compile time (outside any request scope) and cannot carry a per-request nonce, so it is silently blocked and every Alpine component dies. Fix with `alpine_csp=True` (the `@alpinejs/csp` build needs no inline bootstrap) — relaxing the CSP to add `'unsafe-inline'` defeats the nonce and is not the fix. Suspense initial-load scripts are already nonced via the request lifecycle and are exempt. |
