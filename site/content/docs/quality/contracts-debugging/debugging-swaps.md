...
| Empty list looks like loading | Template used `{% if items %}` for a deferred value | Use `{% if items is deferred %}` or `__chirp_defer_pending__` before testing resolved values |
