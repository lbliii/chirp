...
|------|-------|----------|
| `chirp.ext.milo` | `MiloContext`, `MiloContextProvider`, `MiloMCPAppAdapter`, `MiloMCPAppBinding`, `use_milo` | Verifies exact Milo command allowlists, MCP App tool/resource links, and immutable Chirp template/block bindings at freeze; `MiloMCPAppAdapter.render_resource(operation_id)` invokes the bound context provider per read and renders the named block through `Fragment` / `App.render`. |
| `chirp.skill` | `Envelope`, `sign_envelope`, `verify_envelope`, `Skill`, `use_skill`, `Manifest`, `assemble_manifest`, `compute_content_digest`, `SkillRegistry`, `mount_skills`, `DEFAULT_DISCOVERY_PATH`, `DEFAULT_INVOCATION_LOG_PATH`, `mount_console`, `DEFAULT_CONSOLE_PATH`, `ReliabilityScore`, `ReliabilityStore`, `EnvKeystore`, `KeyStatus`, `KEY_STATUS_TOOL`, `SecretLeakError`, `assert_no_secret_leak`, `register_key_status_tool` | Signed skill-tool results with Ed25519 sign/verify; negotiate() emits the wire JSON. Tool handlers are wrapped on mount onto the app MCP registry. use_skill registers the skill as a freeze domain (milo register_domain precedent); at app.freeze() the immutable Manifest is finalized with a content_digest over tool schemas + template sources + public key. `@skill.tool(..., scopes=(...))` enforces AuthSpec(scopes=...) via enforce_auth (declare scopes with app.register_scope). SkillRegistry / mount_skills store skills by name, mount each via use_skill onto one aggregated `/mcp`, register a discovery route (default `/skills`) that lists manifests as JSON, and by default bridge `ToolEventBus` into a live invocation-log `EventStream` at `/invocations/live` (`invocation_log_path=None` disables). Console mount adds hypermedia browse + skill detail pages (Page/Fragment) showing manifest, contract summary, and smoke reliability score; the detail live-log hook sse-connects to the invocation log when mounted. EnvKeystore resolves provider keys from env by name (manifest declares names only); `key-status` reports presence without values and runs a leak guard so secrets never enter Envelope/MCP responses — wire into the console via `EnvKeystore.as_key_status_fn()`. Peer dep: optional `chirp[skill]` (`cryptography`). |
...
does not freeze or mutate the caller-owned Milo CLI or manufacture
request/session state. Resource reads stay caller-owned: the `@cli.ui_resource`
handler should call `adapter.render_resource(operation_id)`, which invokes the
parameterless context provider for that read and renders the bound named block
through Chirp's existing fail-loud `App.render` / `Fragment` surface.
