...
| Suspense resolves to the wrong branch for empty data | Template checks `{% if key %}` after deferred resolution | `defer_falsy` contract warning | Use `{% if key is not none %}` or `"key" in __chirp_defer_pending__` | `examples/standalone/suspense_dashboard` |
