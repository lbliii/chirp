...
| Concurrent Suspense DB loaders stall or contend oddly | More independent defers check out than `pool_size`, or one connection held across sibling defer awaits | Pelt pool exclusive checkout + Suspense concurrent drain (#950) | Acquire/release inside each defer; size `pool_size` to peak concurrent independent checkouts (excess acquires wait, never share) | `examples/standalone/suspense_dashboard`, `tests/test_suspense_pelt_checkouts.py` |
