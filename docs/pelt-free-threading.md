...
| Concurrent Suspense independent defers hold distinct Pelt checkouts; exhaustion queues on `acquire` without sharing | Suspense concurrent defer drain + `Pool.acquire()` | `test_independent_suspense_defers_use_distinct_pelt_checkouts`; `test_pool_exhaustion_queues_acquire_without_sharing_connection`; `test_independent_suspense_pelt_checkouts_under_nogil` (#950) |
| Suspense-like defer fan-out keeps exclusive ownership, rejects double-release, and returns the pool to idle | `Pool.acquire()` / `Pool.release()` under overlapping task-group pages | `test_suspense_shaped_pool_checkout_stress_stays_exclusive_and_idle` (#957); size the pool at least as large as concurrent defer fan-out (Pounce worker sizing docs: #958) |
