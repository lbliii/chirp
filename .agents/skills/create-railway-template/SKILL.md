...
description: Create, publish, update, and verify zero-input Railway templates for Chirp applications. Use for Chirp Railway catalog issues, marketplace-template creation, generated variable defaults, PostgreSQL service wiring, template metadata, disposable proof deployments, publication, or template smoke testing.
---
...
2. Build and verify the application in its own repository. Pin a release tag before using it as template source.
...
- Exercise `/health`, `/ready`, static assets, administrator login, ingress capture, detail rendering, and restart persistence for Hookbox-like applications.
