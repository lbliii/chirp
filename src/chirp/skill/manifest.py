...
provider-key *names* (not secrets), and a content digest. Freeze-time digest
finalization (#974) may replace the provisional digest computed at assemble.
"""
...
def assemble_manifest(
    *,
    ...
    """Build a :class:`Manifest`, computing a provisional content digest if omitted."""
...
def _provisional_digest(
    *,
    ...
    """SHA-256 over canonical identity fields (freeze may replace — see #974)."""
    payload = {
        "name": name,
        "version": version,
        "tools": list(tools),
