...
def test_intent_timeline_rfc_records_private_capture_implementation_scope() -> None:
    ...
    assert "private artifact/comparator implemented by #648" in text
    assert "No changelog: #647 and #648 change private debug/test capture internals only" in text
...
def test_intent_timeline_rfc_cites_current_trace_evidence() -> None:
    ...
    for path in (
        ...
        "src/chirp/server/intent_replay.py",
...
def test_intent_timeline_rfc_preserves_replay_and_privacy_boundaries() -> None:
    ...
    assert "chirp.intent-timeline/1" in text
    assert "public-safe-v1" in text
    assert "perform no imports, requests, route calls, DOM writes, or application mutation" in text
